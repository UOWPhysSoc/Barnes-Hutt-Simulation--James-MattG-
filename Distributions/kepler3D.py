...
class kepler3D:
    ...
    def __init__(self):
        ...
        self.parameters = [
            ...
            {'pName':'Thickness', 'pType':'numeric', 'default':0.1, 'tooltip':'Thickness of gaussian height parallel to axis'},
    ...
    def run(self, imports, dist, G):
        ...
        r0 = float(imports[3])
        z0 = float(imports[4])
        origin = strToVector(imports[5])
        v0 = strToVector(imports[6])
        R = norm(strToVector(imports[7]))
        Rp = perp(R)
        ...
        for i in range(0,n-1):
            ...
            r.rotate(theta,R)
            r *= gammavariate(2,1)
            zMax = z0 * exp(-mag(r)**2 / r0**2)
            z = uniform(-zMax, zMax) * R
            r += z
